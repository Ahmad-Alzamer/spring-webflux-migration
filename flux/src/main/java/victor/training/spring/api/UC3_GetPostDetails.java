...
public class UC3_GetPostDetails {
  ...
  public Mono<GetPostDetailsResponse> getPostDetails(@PathVariable long postId) {
    var postMono = postRepo.findById(postId);
    var commentsFlux = commentRepo.findByPostId(postId)
        .map(CommentResponse::new)
        .collectList();
    return Mono.zip(postMono, commentsFlux)
            .map(tuple -> new GetPostDetailsResponse(tuple.getT1(), tuple.getT2()))
            ;
  }
